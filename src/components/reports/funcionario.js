    ...
    const onSubmit = (event) => {
        ...
        if (anoPeriodo1 && mesPeriodo1) {
            if (!(anoPeriodo2 && mesPeriodo2)) {
                // primeiros dados
                const url = `https://med-backend-dev.herokuapp.com/info/vendas/funcionarios/${mesPeriodo1}/${anoPeriodo1}`
                console.log(url)
                axios.get(url, params)
                    ...
                    .then(data => {
                        ...
                        setDataSet(data.map(entry => ({ nome: entry.cpf, receita: entry.receita})))
                        if (data.length === 0) {
                ...
                // dados comparativos
                const url = `https://med-backend-dev.herokuapp.com/info/vendas/funcionarios/${mesPeriodo2}/${anoPeriodo2}`
                console.log(url)
...
export default connect(() => {}, mapDispatchToProps)(EmployeeReport)